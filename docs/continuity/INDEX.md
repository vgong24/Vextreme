...
**Genuinely open — priority order for the active thread (Session 025 continued):**
1. [ ] **FAB system unification** — in active design/scoping now. Victor wants ONE expandable-FAB framework instead of one-off widgets, so a new orb (theme, map, etc.) is a config registration, not a new file. This retroactively activates two feature slots that have sat reserved-but-unbuilt in `lib/vex-config.js`/`lib/build-vextreme.js`'s `FEATURES` registry since Session 013 (`filename: null`, silently no-op'd): `spiral-fab` (already the default umbrella-trigger feature on nearly every page's viewmodel) and `theme` (dark/light toggle). Real precedent found in `widgets/demo-fab.js`'s own docstring: "Deliberately NOT built as a generalized radial-menu framework yet... If/when a second orb is needed, generalize then" — that threshold is now being crossed for real (existing orbs: `widgets/fab-demo.js` → vextreme-demo.html, `widgets/fab-lang.js` → language wheel; both currently separate hand-positioned widgets with hardcoded pixel offsets to avoid overlapping). Open design question mid-resolution: does the language orb nest inside the spiral-fab's expansion, or stay a sibling orb (since language is a 2-step "expandable" per Victor's own action-vs-dropdown distinction, not a 1-tap action like theme/close)?
2. [ ] `Feature.MAP` — a live-page FAB linking back to `pages/terrain-map.html` — the natural next increment for the terrain map, touches the shared `FEATURES` registry (real production surface). Likely becomes one orb registration in the new FAB system above rather than a standalone widget, once that system exists. Distinct from the existing `widgets/fab-demo.js` orb, which links to `vextreme-demo.html` (architecture demo), not the terrain map — worth confirming with Victor whether his wireframe's "map" orb means this (terrain-map.html) or the existing demo orb, or both.
3. [ ] **Mobile support for `pages/terrain-map.html`** — next PR after the FAB system work. Pan/zoom is currently mouse-event-only (no touch/pinch); the detail panel is a fixed right-side sidebar with no responsive breakpoint. Proposed approach (not yet started): unify to Pointer Events (`pointerdown`/`pointermove`/`touch-action: none`) for pan, add pinch-to-zoom via two-pointer distance delta, and a ~768px breakpoint that repositions the detail panel to a bottom sheet via CSS only (same panel logic). Health check likely a Playwright pass at a mobile viewport, not a build-time drift detector — needs its own design pass once reached.
