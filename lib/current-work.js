...
const workCoordination = require('./work-coordination');
...
function lookupRepoOpenPrs(ownerRepo) {
  ...
  const args = ['pr', 'list', '--state', 'open', '--json', 'number,title,body,isDraft,headRefName,baseRefName,url'];
...
function buildReport() {
  ...
  const coordinationLiveStatus = ['found', 'none found'].includes(openPr.status) ? 'available' : openPr.status;
  const coordination = workCoordination.inspectPullRequests(
    openPr.prs,
    workCoordination.loadPolicy(),
  );
  ...
  const report = {
    ...
    openPr,
    coordination: { liveStatus: coordinationLiveStatus, ...coordination },
  };
...
function renderReport(report) {
  ...
  lines.push('', 'Work coordination:');
  lines.push(...workCoordination.renderClaims(report.coordination, report.coordination.liveStatus));
