 ...
 * of this architecture." The regression classes this checks for were
 ...
 *                      Full-viewport authored surfaces (90vh+) are included:
 *                      wrapping those changes their composition even when
 *                      they do not declare an explicit wide pixel value.
 ...
 *   4. page-action collision — the page declares fixed top-right chrome but
 *                      has not marked it data-vex-page-action. The marker
 *                      opts the control into site-nav.css's below-nav lane,
 *                      keeping it clear of the nav action rail.
 *
...
// Full-viewport heroes and tools rely on the page surface itself, not a
// generic prose wrapper. The 90vh threshold catches deliberate viewport
// composition (Phantom's 92vh hero, terrain's 100vh app) without treating
// ordinary section sizing as a global layout signal.
function usesViewportHeightLayout(html) {
  const re = /([^(])(?:min-)?height:\s*(\d+(?:\.\d+)?)(?:d|s|l)?vh/g;
  let m;
  while ((m = re.exec(html))) {
    if (parseFloat(m[2]) >= 90) return true;
  }
  return false;
}
...
function hasFixedTopRightChrome(html) {
  const blocks = html.match(/[^{}]+\{[^{}]*position:\s*fixed[^{}]*\}/g) || [];
  return blocks.some((block) =>
    /\btop\s*:/.test(block) &&
    /\bright\s*:/.test(block) &&
    !/\bleft\s*:/.test(block) &&
    !/\bbottom\s*:/.test(block)
  );
}

function hasPageActionMarker(html) {
  return /\bdata-vex-page-action\b/.test(html);
}
...
function auditPage(slug, html) {
  ...
  const viewportHeight = usesViewportHeightLayout(html);
  if (!hasBodyWrapOptOut(html)) {
    if (widest !== null && widest > WRAP_CAP_PX) {
      ...
      findings.push({
        type: 'wide-layout',
        detail: `full-viewport layout (90vh+), composition changed by the ${WRAP_CAP_PX}px wrap, no bodyWrap:false override`,
      });
    }
  }
  ...
  if (hasFixedTopRightChrome(html) && !hasPageActionMarker(html)) {
    findings.push({
      type: 'page-action-collision',
      detail: 'fixed top-right page chrome has no data-vex-page-action marker for the below-nav lane',
    });
  }
