...
for (const [key, { value, file }] of Object.entries(source)) {
  ...
  // BLOCK: missing EN
  if (!value.en || !value.en.text) {
    console.error(`[strings-check] BLOCK  Missing EN text for key: "${key}" in ${file}`);
    blocked = true;
    continue;
  }
  ...
  if (byFrom[key]) {
    ...
    if (newKey) {
      console.warn(`[strings-check] REMAP  "${key}" → "${newKey}" (${migration.reason})`);
      ...
      console.warn(`[strings-check] REMAP  "${key}" was intentionally deleted per migrations.json — removing`);
      const fileData = getFile(file);
      delete fileData[key];
    }
    continue;
  ...
  if (manifest[key]) {
    const currentHash = sha256Short(JSON.stringify(value.en));
    if (manifest[key].enHash && manifest[key].enHash !== currentHash) {
      const langs = Object.keys(value).filter(l => l !== 'en');
      console.warn(`[strings-check] WARN   Stale translations for "${key}" (EN changed). Affected langs: ${langs.join(', ')}`);
      const fileData = getFile(file);
      for (const lang of langs) {
        if (fileData[key] && fileData[key][lang] && !fileData[key][lang]._stale) {
          fileData[key][lang]._stale = true;
        }
      }
    }
  }
...
for (const key of Object.keys(manifest)) {
  ...
  if (manifestEntry.enHash) {
    const matchingKey = Object.entries(source).find(([k, { value }]) => {
      ...
      return sha256Short(JSON.stringify(value.en)) === manifestEntry.enHash;
    });
    if (matchingKey) {
  ...
  console.warn(`[strings-check] QUARANTINE "${key}" — in manifest but not in source and no migration. Moving to orphans.json.`);
...
if (blocked) {
  console.error('[strings-check] BLOCKED — fix missing EN strings before compiling.');
  process.exit(1);
} else {
