...
// Arc links deep-link into archives.html sections using IDs set by build-archives.js
function arcDeepLink(key) {
  return `${GH_BASE}/archives.html#arc-${key.replace(/_/g, '-')}`;
}

const arcHubs = ARC_ORDER.map(key => {
  const def = arcsDef[key];
  if (!def) return null;
  return { key, title: def.parent.title, url: arcDeepLink(key) };
}).filter(Boolean);
