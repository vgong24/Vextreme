 ...
 * Shows every active institutional destination, Terrain, arc hub links, and
 * the last 5 nodes added to nodes.json (by id).
 *
 ...
 *   role      : generates index.html — the root archive landing page, all active institutional destinations, Terrain/arc links, and 5 most recently added nodes
 *   reads     : data/nodes.json
 *               data/arcs-v2.json
 *               config/institutional-surfaces.json (all active institutional discovery)
 *               lib/check-institutional-surfaces.js (surfacesByState)
 *               data/strings/compiled/scopes/{category}/{scope}.en.json (root discovery labels)
 *               pages∕*.html (existence check, per slug)
 *   writes    : index.html
 *   loaded-by : .github/workflows/build-index.yml
 *   tested-by : (none)
 *
 *   CHANGE MAP — if you touch X here, also check:
 *     local :root token values changed:
 *       - consider migrating to styles/design-system.css's shared tokens instead, per docs/architecture/12-design-system.md — this file's local block is the last one of its kind
 *     'most recent 5 nodes' selection logic changed:
 *       - confirm it still reads by nodes.json id order, not data/index.json's arc-derived order — they answer different questions
 *     institutional lifecycle or discovery label changes:
 *       - config/institutional-surfaces.json — every active existing surface is advertised
 *       - data/strings/source/institution.json — discovery labels remain canonical string projections
 *       - tests/54-institutional-surfaces.test.js root discoverability assertion
 * LATTICE:END
 */
...
const { surfacesByState } = require('./check-institutional-surfaces');
...
function activeInstitutionalLinks() {
  const registryPath = path.join(ROOT, 'config', 'institutional-surfaces.json');
  if (!fs.existsSync(registryPath)) return [];
  const registry = JSON.parse(fs.readFileSync(registryPath, 'utf8'));
  return surfacesByState(registry, 'active').map(surface => {
    if (!isPorted(surface.slug)) return null;
    const bundlePath = path.join(
      ROOT, 'data', 'strings', 'compiled', 'scopes', surface.strings.category,
      `${surface.strings.scope}.en.json`
    );
    const bundle = JSON.parse(fs.readFileSync(bundlePath, 'utf8'));
    const label = bundle[surface.discovery.rootLabelKey]?.text;
    if (typeof label !== 'string' || !label.trim()) {
      throw new Error(`${surface.slug} discovery label ${surface.discovery.rootLabelKey} is absent from ${bundlePath}`);
    }
    return { slug: surface.slug, label };
  }).filter(Boolean);
}

const portedCount = nodes.filter(n => n.id !== null && isPorted(n.slug)).length;
const totalCount  = nodes.filter(n => n.id !== null).length;
const institutionalLinks = activeInstitutionalLinks()
  .map(surface => `  <a href="${pageUrl(surface.slug)}">${surface.label}</a>`)
  .join('\n');
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .terrain-card:hover { border-color: var(--ember); }
  .terrain-card-kicker { font-family: var(--mono); font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; color: var(--ember); margin-bottom: 8px; }
  .terrain-card-title { font-family: var(--sans); font-size: 16px; color: var(--text); margin-bottom: 6px; }
  .terrain-card-body { font-family: var(--sans); font-size: 13px; color: var(--muted); line-height: 1.6; }
...
<div class="section-label">
  ...
  <a href="${TERRAIN_URL}">Open map →</a>
</div>

<a class="terrain-card" href="${TERRAIN_URL}">
  <div class="terrain-card-kicker">New atlas</div>
  <div class="terrain-card-title">Terrain Map</div>
  <div class="terrain-card-body">
    Code lattice, health status, dependency edges, and screenshot-backed live screens in one navigable map.
  </div>
</a>
