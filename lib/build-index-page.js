 ...
 * Shows the active institutional home, Terrain, arc hub links, and the last 5
 * nodes added to nodes.json (by id).
 *
 ...
 *   role      : generates index.html — the root archive landing page, active institutional-home route, Terrain/arc links, and 5 most recently added nodes
 *   reads     : data/nodes.json
 *               data/arcs-v2.json
 *               config/institutional-surfaces.json (active institution-home discovery)
 ...
 *     institutional-home lifecycle or purpose changes:
 *       - config/institutional-surfaces.json — only an active existing institution-home is advertised
...
function activeInstitutionalHomeSlug() {
  const registryPath = path.join(ROOT, 'config', 'institutional-surfaces.json');
  if (!fs.existsSync(registryPath)) return null;
  const registry = JSON.parse(fs.readFileSync(registryPath, 'utf8'));
  const home = surfacesByState(registry, 'active')
    .find(surface => surface.purpose === 'institution-home');
  return home && isPorted(home.slug) ? home.slug : null;
}

const portedCount = nodes.filter(n => n.id !== null && isPorted(n.slug)).length;
const totalCount  = nodes.filter(n => n.id !== null).length;
const institutionalHomeSlug = activeInstitutionalHomeSlug();
const institutionalHomeLink = institutionalHomeSlug
  ? `  <a href="${pageUrl(institutionalHomeSlug)}">About Vextreme</a>\n`
  : '';
...
<nav class="nav-strip">
${institutionalHomeLink}  <a href="${GH_BASE}/archives.html">Archives</a>
