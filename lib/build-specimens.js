...
// stringScopeGlobals — page-specific config fab-lang.js reads at mount time
// (which string category/scopes this page's language switch covers). This
// is real per-page data, not infrastructure — it stays. What used to live
// here too (lang-fab.js/demo-fab.js <script> tags) does not: lib/vextreme.js
// now auto-loads the full current spiral-FAB set (vex-fab.js + fab-lang.js +
// fab-theme.js + fab-map.js) for any page that includes shell.js, so a
// generator emitting its own widget <script> tags would be exactly the
// "duplicate the same category of wiring in multiple places" pattern this
// repo's own architecture avoids elsewhere. See lib/vextreme.js's
// loadFabWidgets() for why fab-demo.js is excluded (deprecated) and
// fab-analysis.js is excluded (needs real per-page analysis data these
// fixtures don't have).
function stringScopeGlobals(scopes) {
...
// fabWidgets: { theme: false } — this page's own <html data-theme="dashboard">
// is a static, intentional CSS variant selector (see SHARED_CSS), not a
// dark/light toggle state. fab-theme.js's own mount() unconditionally
// overwrites document.documentElement's data-theme with its saved/default
// "light"/"dark" value on load (checked directly in widgets/fab-theme.js,
// not assumed) — that would silently clobber "dashboard" and break this
// page's whole visual identity the instant it mounted. lang/map still load.
...
<body>
${body}
${stringScopeGlobals(scope)}
<script>window.VEXTREME_OVERRIDE = { bodyWrap: false, fabWidgets: { theme: false } };</script>
<script src="${CDN_BASE}/lib/shell.js"></script>
</body>
</html>`;
}
