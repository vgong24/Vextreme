...
// Mirrors the path derivation rule in lib/strings-compile.js and widgets/lang-fab.js.
// Dots in scope names become directory segments within the category directory.
function scopeRelPath(scope, category) {
  const segs = scope.split('.');
  return [category, ...segs.slice(0, -1), segs[segs.length - 1]].join('/');
}
...
<script>
(function () {
  var url = '${CDN_BASE}/data/strings/compiled/scopes/${scopeRelPath(missScope, 'demo')}.ja.json?v=' + Date.now();
