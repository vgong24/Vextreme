 ...
 *   - Core modules (sw-register.js) — always included, infrastructure not features
...
// Core modules — always included in every God Script regardless of viewmodel features.
// These are infrastructure, not features: every page that loads a God Script gets them.
// Order matters: core modules run before feature modules.
const CORE_MODULES = [
  'sw-register.js',  // Service Worker registration — activates offline caching on first load
];
...
function assembleGodScript(slug, viewmodel, options) {
  ...
  // Collect core module sources — always included, order preserved
  const coreParts = [];
  for (const filename of CORE_MODULES) {
    const modulePath = path.join(widgetsDir, filename);
    if (!fs.existsSync(modulePath)) {
      logger.warn({ code: CODES.BUILD_MISSING_FEATURE, message: `Core module not found: ${filename}`, path: modulePath });
      continue;
    }
