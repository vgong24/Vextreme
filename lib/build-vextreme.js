 ...
 *
 * LATTICE
 *   role      : God Script assembler — reads index + string bundles, writes one
 *               self-contained dist/ file per page; no page fetches anything
 *               at load time that isn't already inlined or cached
 *   reads     : data/index.json (slugMap, supportedLangs — run build-index.js first)
 *               data/viewmodels.json (demo/specimen overrides)
 *               data/strings/compiled/scopes/{category}/{scope}.en.json (inlined EN strings)
 *               widgets/sw-register.js, widgets/fab-lang.js, widgets/fab-demo.js
 *               lib/vextreme-index-v2.js (arc-nav feature, srcDir: LIB_DIR)
 *               lib/vex-config.js (Feature, DEFAULT_LANGUAGE, scopeRelPath)
 ...
 *   FEATURES registry entry added → lib/vex-config.js Feature.* constant,
 *                                    blueprint.json features[], config/features/{id}.json,
 *                                    tests/08 (add coverage for new feature output)
 *   assembleGodScript() output    → every window.VEX_* consumer:
 *                                    widgets/fab-lang.js (reads VEX_STRING_SCOPES/CATEGORY),
 *                                    lib/vextreme-index-v2.js (reads VEX_STRINGS_EN fast path),
 *                                    tests/08 (all ASSEMBLER: assertions)
 *   Scope bundle path convention  → must match lib/strings-compile.js output path
 *                                    AND widgets/fab-lang.js fetch URL construction
 *   dist/ output filenames        → lib/build-sw.js PRECACHE_URLS (sw.js regenerates separately)
 */
...
const FEATURES = {
  ...
  [Feature.SPIRAL_FAB]:{ filename: null,                    default: false },
  [Feature.THEME]:     { filename: null,                    default: false },
};
...
if (require.main === module) {
  ...
  for (const { slug, viewmodel } of allSlugs) {
    ...
    const godScript  = assembleGodScript(slug, viewmodel, { supportedLangs: index.supportedLangs || [] });
