 ...
 *               lib/audit-pages.js (SKIP_PAGES for generated system pages; AUTO_DISCOVERY_EXCLUSIONS + getPageSlugs for record discovery)
...
const { SKIP_PAGES, AUTO_DISCOVERY_EXCLUSIONS, getPageSlugs } = require('./audit-pages');
...
const PAGE_HEALTH_IN  = path.join(ROOT, 'data', 'page-health.json');
...
const departmentsDef = fs.existsSync(DEPARTMENTS_IN)
  ...
  : {};
const pageHealth = fs.existsSync(PAGE_HEALTH_IN)
  ? JSON.parse(fs.readFileSync(PAGE_HEALTH_IN, 'utf8'))
  : { summary: {}, pages: {} };
...
const discoveredNodes = discoverOrphanNodes(getPageSlugs(), curatedSlugs, AUTO_DISCOVERY_EXCLUSIONS, viewmodelSlugs, readPageFromDisk, departmentsDef);
...
function healthBadges(slug) {
  const page = pageHealth.pages[slug];
  if (!page) return '';
  const badges = [
    { label: page.health.state === 'healthy' ? 'OK' : (page.health.state === 'critical' ? 'BLOCK' : 'CHECK'), kind: page.health.state },
    { label: page.fab.delivery === 'missing' ? 'NO FAB' : 'FAB', kind: page.fab.delivery === 'missing' ? 'missing' : 'present' },
    { label: page.theme.visibleStyles ? 'THEME' : 'NO THEME', kind: page.theme.visibleStyles ? 'present' : 'missing' },
    { label: page.analysis.control ? 'ANALYSIS' : 'NO ANALYSIS', kind: page.analysis.control ? 'present' : 'missing' },
    { label: page.discovery.metaDescription.length ? 'META' : 'NO META', kind: page.discovery.metaDescription.length ? 'present' : 'missing' },
  ];
  return `<span class="cell-health">${badges.map(b => `<span class="health-chip health-chip--${b.kind}">${b.label}</span>`).join('')}</span>`;
}
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cell-health { display:flex; gap:3px; flex-wrap:wrap; margin-top:auto; padding-top:3px; }
  .health-chip { font-family:var(--mono); font-size:7.5px; line-height:1; padding:3px 4px; border-radius:2px; border:1px solid var(--border); color:var(--muted); }
  .health-chip--healthy, .health-chip--present { color:#76b77a; border-color:#315b34; }
  .health-chip--attention { color:#d7a846; border-color:#70571f; }
  .health-chip--critical, .health-chip--missing { color:#c86a64; border-color:#6d302c; }

  .health-summary { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:8px; margin:0 0 32px; }
  .health-stat { border:1px solid var(--border); border-radius:5px; padding:12px; background:var(--surface); }
  .health-stat strong { display:block; font-family:var(--mono); font-size:18px; font-weight:500; color:var(--text); }
  .health-stat span { font-family:var(--mono); font-size:9px; color:var(--muted); text-transform:uppercase; letter-spacing:.06em; }
  @media (max-width:640px){ .health-summary{grid-template-columns:repeat(2,minmax(0,1fr));} }
...
<section class="health-summary" aria-label="Page capability health">
  <div class="health-stat"><strong>${pageHealth.summary.healthy || 0}</strong><span>healthy</span></div>
  <div class="health-stat"><strong>${pageHealth.summary.attention || 0}</strong><span>needs attention</span></div>
  <div class="health-stat"><strong>${pageHealth.summary.critical || 0}</strong><span>blocked / isolated</span></div>
  <div class="health-stat"><strong>${pageHealth.summary.withFab || 0} / ${pageHealth.summary.total || 0}</strong><span>FAB delivery</span></div>
</section>
