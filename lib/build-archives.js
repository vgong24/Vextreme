...
// Load compiled per-language bundles and merge into { key: { lang: value } }
// for embedding in the page's inline i18n resolver.
const COMPILED_DIR = path.join(ROOT, 'data', 'strings', 'compiled');
function loadCompiledStrings() {
  const merged = {};
  for (const file of fs.readdirSync(COMPILED_DIR)) {
    if (!file.startsWith('strings.') || !file.endsWith('.json')) continue;
    const lang = file.replace('strings.', '').replace('.json', '');
    const bundle = JSON.parse(fs.readFileSync(path.join(COMPILED_DIR, file), 'utf8'));
...
const strings = loadCompiledStrings();
...
<header class="site-header">
  <div class="site-title" data-i18n="common.label.site-title">Vextreme</div>
  <h1 class="site-sub" data-i18n="archives.heading.page-title">The Archive</h1>
  <div class="site-meta">
    <span data-i18n="common.status.pages-live" data-i18n-vars='{"ported":${totalPorted},"total":${totalNodes}}'>${totalPorted} of ${totalNodes} pages live</span>
    <span data-i18n="common.status.remaining" data-i18n-vars='{"count":${totalNodes - totalPorted}}'>${totalNodes - totalPorted} remaining</span>
    <span data-i18n="common.status.built-on" data-i18n-vars='{"date":"${builtAt}"}'>Built ${builtAt}</span>
  </div>
</header>

<div class="global-bar-wrap">
  <div class="global-bar-label">
    <span data-i18n="archives.label.overall-progress">Overall progress</span>
...
<div class="legend">
  <div class="legend-item"><div class="legend-dot legend-dot--ported"></div> <span data-i18n="common.label.page-live">Page live</span></div>
  <div class="legend-item"><div class="legend-dot legend-dot--missing"></div> <span data-i18n="common.label.not-yet-ported">Not yet ported</span></div>
</div>

${arcSections}

<div id="slugPopover" class="slug-popover" style="display:none">
  <div class="slug-popover-label" data-i18n="common.label.slug">Slug</div>
  <div class="slug-popover-value" id="slugPopoverSlug"></div>
  <div class="slug-popover-filename" id="slugPopoverFile"></div>
  <button class="slug-popover-copy" id="slugPopoverBtn" data-i18n="common.button.copy-filename">Copy filename</button>
</div>
...
(function () {
  ...
  function t(key, vars) {
    var entry = STRINGS[key] && (STRINGS[key][lang] || STRINGS[key]['en']);
    var str = (entry && entry.text) || key;
  ...
  window.setLang = function (code) {
    ...
    copyBtn.textContent = t('common.button.copy-filename');
  };
  ...
  copyBtn.addEventListener('click', function (e) {
    ...
    navigator.clipboard.writeText(currentSlug + '.html').then(function () {
      copyBtn.textContent = t('common.button.copied');
      setTimeout(function () { copyBtn.textContent = t('common.button.copy-filename'); }, 1500);
    });
  });
}());
</script>
