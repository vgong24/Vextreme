...
// Each entry gets:
//   dateISO  — computed YYYY-MM-DD for range queries
//   arcKeys  — re-sorted by arc priority so the browser renders in correct
//               order without needing its own priority table
const slugMap = {};
...
// Arc display metadata (title + parent URL) extracted from arcs-v2.json.
// Stored in index.json so the browser lib has no hard-coded arc tables.
const arcMeta = {};
for (const [arcName, arcDef] of Object.entries(arcsDef)) {
