...
(function () {
  ...
  function loadStrings(onReady) {
    ...
    req.onload = function () {
      if (req.status === 200) {
        ...
        } catch (e) { console.warn('[vextreme-v2] Failed to parse strings bundle', e); }
      } else {
        console.warn('[vextreme-v2] strings bundle returned HTTP ' + req.status);
      }
      onReady();
    };
    req.onerror = function () {
      console.warn('[vextreme-v2] Failed to fetch strings bundle — UI text will fall back to keys');
      onReady();
    };
    req.send();
  }
  ...
  var RENDERERS = {
    ...
    dots: function (v) {
      ...
      var prevText = t('common.nav.prev');
      var nextText = t('common.nav.next');
      var prev  = v.prevUrl ? '<a href="' + v.prevUrl + '" class="arc-nav-arrow" aria-label="' + t('common.nav.prev') + '">' + prevText + '</a>'
                            : '<span class="arc-nav-arrow disabled" aria-hidden="true">' + prevText + '</span>';
      var next  = v.nextUrl ? '<a href="' + v.nextUrl + '" class="arc-nav-arrow" aria-label="' + t('common.nav.next') + '">' + nextText + '</a>'
  ...
  function renderArcNav(lattice, mountEl) {
    ...
    mountEl.innerHTML = '<div class="arc-nav">'
      + rows
      + '<div class="arc-nav-current"><span class="arc-nav-current-label">' + t('common.label.you-are-here') + '</span>: ' + lattice.node.title + '</div>'
      + '</div>';
  }
  ...
  function mount() {
    ...
    function tryRender() {
      ...
      var lattice = getLatticeView(slug, indexData);
