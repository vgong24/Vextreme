...
const { run: checkDocumentRoutes } = require('./check-document-routes');
...
function evaluateInvariants(suite, projection, root = ROOT) {
  ...
  return [
    result(
      'document-routes-clean',
      !suite.invariants.requireDocumentRoutes || checkDocumentRoutes(root).issues.length === 0,
      'every docs file has exactly one declared route'
    ),
    result(
