 ...
 * v2.0.0
 *
 * The unified loader. One function, one optional argument.
 * Works on every environment. Self-configuring from slug alone.
 *
 * USAGE — the simplest form (covers most pages):
 *
 *   <div id="arcNavMount"></div>
 *   <script src=".../shell.js?v=4"></script>
 *
 *   That's it. Slug detected from URL. Template detected from pages.json.
 ...
 *
 * ADDING NEW CONFIG FIELDS IN THE FUTURE:
 *   Add the field to resolveConfig() with a default value.
 *   The function call never changes — only the schema grows.
 *   Existing call sites that don't pass the field are unaffected.
 *
 * ENVIRONMENT STYLESHEET MAP:
 *   squarespace  → design-system.css, arc-nav.css, squarespace-overrides.css
 *   github_pages → design-system.css, arc-nav.css, site-nav.css
 *   local        → design-system.css, arc-nav.css, site-nav.css
 *   + template CSS resolved from pages.json after data loads
 *
 * FORK USAGE:
 *   VEXTREME({ baseUrl: 'https://cdn.jsdelivr.net/gh/yourname/yourrepo@main' });
 */
