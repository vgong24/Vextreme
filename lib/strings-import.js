...
const { Language } = require('./vex-config');
...
for (const csvFile of csvFiles) {
  ...
  for (const row of rows) {
    ...
    // Clear stale flag on import; write into .strings namespace
    if (!fileData[key].strings) fileData[key].strings = {};
    fileData[key].strings[lang] = translation;
    importCount++;
